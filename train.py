# ----------------- Losses, Initialization & Training ----------------- #
...
def discriminator_loss(disc, true_samples, gen_samples, param):
    """
    ...
    :param gen_samples: samples from current Generator
...
def generator_loss(disc, gen, noise, param):
    ...
    :param noise: samples from latent distribution
    :param param: AttrDict with additional parameters
...
def trainer(disc, gen, disc_optimizer, gen_optimizer, param):
    ...
    # Prepare lists for training & visualization #
    gen_losses = []
    disc_losses = []
    W_dist = []
    decision_boundaries = []
    ...
    # initialize LR Scheduler (if applicable, specify as desired) #
    ...
    for epoch in tqdm(range(param.n_epochs + 1)):
        for _ in range(param.n_disc_train):
            ...
            if param.gan_type == "dirac" and param.dirac_noise == True:
                gen_samples += 1. * torch.randn_like(gen_samples, device=param.device)

            # --- Update discriminator --- #
            # Zero out the gradients before backpropagation
            disc_optimizer.zero_grad()
            # calculate discriminator loss
            disc_loss = discriminator_loss(disc, true_samples=true_samples, gen_samples=gen_samples, param=param)
            # Backpropagation
